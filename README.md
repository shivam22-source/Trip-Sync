...
```txt
Travel Buddy is a full-stack trip collaboration MVP where users can create trips, request to join trips, admins can approve members, and accepted members can chat in realtime.
```

Frontend explanation:

```txt
I used React Router for navigation and protected routes, a centralized API service for token-based requests, controlled forms for auth/profile/trip creation, conditional rendering for admin/member states, and Socket.io client for realtime chat.
```

Backend explanation:

```txt
The backend is built with Express and MongoDB. JWT middleware protects private APIs. Mongoose models store users, trips, members, and messages. Socket.io handles realtime trip chat using authenticated sockets and trip rooms.
```

Most important learning:

```txt
This phase taught me how frontend becomes real engineering: auth persistence, protected routes, async state handling, role-based rendering, REST workflows, and realtime state synchronization.
```
